...
public sealed class RaftHttpClusterTests : RaftTest
{
    ...
    public static async Task CommunicationWithLeader()
    {
        ...
        await GetLocalClusterView(host1).LeaderRouter.SendSignalAsync(new TextMessage("Message to leader", "simple"), TestToken);
        ...
        foreach (var box in new[] { box1, box2, box3 })
            ...
            {
                success = true;
                Equal("Message to leader", await response.ReadAsTextAsync(TestToken));
                break;
            }
    ...
    public static async Task MessageExchange(string protocolPath)
    {
        ...
        //request-reply test
        var response = await client.SendTextMessageAsync(CreateBufferedMessageAsync, "Request", "Ping", token: TestToken);
        True(response.IsReusable);
        NotNull(response);
        Equal("Reply", response.Name);
        Equal("Pong", await response.ReadAsTextAsync(TestToken));

        //one-way message
        await client.SendTextSignalAsync("OneWayMessage", "Hello, world", token: TestToken);
        True(messageBox.TryDequeue(out response));
        NotNull(response);
        Equal("Hello, world", await response.ReadAsTextAsync(TestToken));

        //one-way large message ~ 1Mb
        await client.SendSignalAsync(new BinaryMessage(new byte[1024 * 1024], "OneWayMessage"), false, TestToken);
        //wait for response
        for (var timeout = new Timeout(TimeSpan.FromMinutes(1)); !messageBox.TryDequeue(out response); timeout.ThrowIfExpired())
    ...
    public static async Task TypedMessageExchange()
    {
        ...
        // duplex messages
        var result = await typedClient.SendMessageAsync<AddMessage, ResultMessage>(new() { X = 40, Y = 2 }, TestToken);
        Equal(42, result.Result);

        result = await typedClient.SendMessageAsync<SubtractMessage, ResultMessage>(new() { X = 40, Y = 2 }, TestToken);
        Equal(38, result.Result);

        // one-way message
        Equal(0, messageBox.Result);

        await typedClient.SendSignalAsync<ResultMessage>(new() { Result = 42 }, TestToken);
    ...
    public static async Task FailureDetection()
    {
        ...
        Equal(new UriEndPoint(GetLocalClusterView(host1).LocalMemberAddress), 
            (await GetLocalClusterView(host1).WaitForLeaderAsync(InfiniteTimeSpan, TestToken)).EndPoint, UriEndPoint.Comparer);
        ...
        await host3.StopAsync(TestToken);
        await memberGoneTask.Task.WaitAsync(TestToken);
    ...
    public static async Task StandbyMode()
    {
        ...
        // suspend two nodes
        True(await GetLocalClusterView(host2).EnableStandbyModeAsync(TestToken));
        True(GetLocalClusterView(host2).Standby);
        True(await GetLocalClusterView(host3).EnableStandbyModeAsync(TestToken));
        ...
        False((await GetLocalClusterView(host1).WaitForLeaderAsync(InfiniteTimeSpan, TestToken)).IsRemote);

        // resume two nodes
        await GetLocalClusterView(host2).RevertToNormalModeAsync(TestToken);
        False(GetLocalClusterView(host2).Standby);
        await GetLocalClusterView(host3).RevertToNormalModeAsync(TestToken);
    ...
    public static async Task RegressionIssue108()
    {
        ...
        using (var host1 = CreateHost<Startup>(3262, config1))
        {
            ...
            await AssertLeadershipAsync(UriEndPoint.Comparer,
                GetLocalClusterView(host1),
                GetLocalClusterView(host2),
                GetLocalClusterView(host3));
    ...
    public async Task ClusterRecovery()
    {
        ...
        using (var host1 = CreateHost<Startup>(3262, config1))
        {
            ...
            // add two nodes to the cluster
            await GetLocalClusterView(host1).AddMemberAsync(GetLocalClusterView(host2).LocalMemberAddress, TestToken);
            await GetLocalClusterView(host2).Readiness.WaitAsync(TestToken);

            await GetLocalClusterView(host1).AddMemberAsync(GetLocalClusterView(host3).LocalMemberAddress, TestToken);
            await GetLocalClusterView(host3).Readiness.WaitAsync(TestToken);
        ...
        do
        {
            leader2 = await GetLocalClusterView(host2).WaitForLeaderAsync(DefaultTimeout, TestToken);
            leader3 = await GetLocalClusterView(host3).WaitForLeaderAsync(DefaultTimeout, TestToken);
        }
        while (UriEndPoint.Comparer.Equals(oldLeader, leader2.EndPoint) || EndPointFormatter.Equals(oldLeader, leader3.EndPoint));
    ...
    public async Task RegressionIssue153()
    {
        ...
        do
        {
            leader2 = await GetLocalClusterView(host2).WaitForLeaderAsync(InfiniteTimeSpan, TestToken);
            leader3 = await GetLocalClusterView(host3).WaitForLeaderAsync(InfiniteTimeSpan, TestToken);
        }
        while (UriEndPoint.Comparer.Equals(oldLeader, leader2.EndPoint) || EndPointFormatter.Equals(oldLeader, leader3.EndPoint));
